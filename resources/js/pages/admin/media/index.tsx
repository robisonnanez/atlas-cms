...
export default function MediaIndex({ media }: { media: MediaRecord[] }) {
    ...
    const removeMedia = (row = selectedMedia) => {
        ...
        if (!window.confirm(`Delete the media asset "${row.filename}"?`)) {
            return;
        }

        router.delete(`/admin/media/${row.id}`, {
            preserveScroll: true,
            onSuccess: () => setSelectedMediaId(null),
        });
    };
                ...
                <section className="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-sm">
                    <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                        <div className="space-y-2">
                            ...
                            <p className="max-w-2xl text-sm text-slate-600">Upload assets, search the library, refine metadata and retire outdated files from the same workspace.</p>
                        </div>
                            ...
                            <CardHeader>
                                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                                    <div>
                                        ...
                                        <CardDescription>Edit the selected file metadata or remove it from the library.</CardDescription>
                                    </div>
                                    {selectedMedia ? <Button label="Delete asset" icon="pi pi-trash" severity="danger" outlined onClick={() => removeMedia()} className="rounded-full" /> : null}
                                </div>
                            </CardHeader>
                            <CardContent>
                                {selectedMedia ? (
                                    <form onSubmit={(event) => {
                                        ...
                                        <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
                                            {selectedMedia.mime_type.startsWith('image/') && selectedMedia.metadata?.url ? (
                                                <img src={selectedMedia.metadata.url} alt={selectedMedia.filename} className="h-56 w-full rounded-2xl object-cover" />
                                            ) : (
                                                <div className="flex h-40 items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-white text-sm text-slate-500">{selectedMedia.mime_type}</div>
                                            )}
                                        </div>

                                        <div className="grid gap-2 text-sm text-slate-600">
                                            <div className="flex items-center justify-between"><span>Filename</span><span className="font-medium text-slate-900">{selectedMedia.filename}</span></div>
                                            <div className="flex items-center justify-between"><span>MIME type</span><span className="font-medium text-slate-900">{selectedMedia.mime_type}</span></div>
                                            <div className="flex items-center justify-between"><span>Size</span><span className="font-medium text-slate-900">{selectedMedia.size ? `${Math.max(1, Math.round(selectedMedia.size / 1024))} KB` : 'Unknown'}</span></div>
                                        </div>
