...
export default function NavigationManagementPage() {
    ...
    const showToast = (severity: 'success' | 'warn', detail: string) => {
        toast.current?.show({ severity, summary: severity === 'success' ? 'OK' : t('Validación', 'Validation'), detail, life: 2500 });
    };

    const submitModule = () => {
        if (editingModule.idModulos === 0 || !editingModule.translations?.es?.trim()) {
            showToast('warn', t('Completa el ID y el nombre en español del módulo.', 'Complete the module ID and Spanish name.'));
            return;
        }
        ...
        if (page.props.modules.some((module) => module.idModulos === editingModule.idModulos)) {
            router.put(`/admin/navigation/modules/${editingModule.idModulos}`, payload, { preserveScroll: true, onSuccess: () => showToast('success', t('Módulo actualizado.', 'Module updated.')) });
        } else {
            router.post('/admin/navigation/modules', payload, { preserveScroll: true, onSuccess: () => showToast('success', t('Módulo creado.', 'Module created.')) });
        }
    ...
    const submitMenu = () => {
        if (!editingMenu.idModulos || !editingMenu.translations?.es?.trim() || !editingMenu.url.trim()) {
            showToast('warn', t('Completa módulo, nombre en español y URL.', 'Complete module, Spanish label and URL.'));
            return;
        }
        ...
        if (editingMenu.id) {
            router.put(`/admin/navigation/menus/${editingMenu.id}`, payload, { preserveScroll: true, onSuccess: () => showToast('success', t('Menú actualizado.', 'Menu updated.')) });
        } else {
            router.post('/admin/navigation/menus', payload, { preserveScroll: true, onSuccess: () => showToast('success', t('Menú creado.', 'Menu created.')) });
        }
    ...
    return (
        <>
            <Head title={t('Gestión de navegación', 'Navigation management')} />
            ...
            <div className="space-y-6">
                <section className="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-sm">
                    <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
                        <div className="space-y-3">
                            ...
                            <p className="max-w-2xl text-sm text-slate-600">{t('Administra la estructura del panel y define etiquetas en español e inglés para una navegación consistente.', 'Manage panel structure and define Spanish and English labels for consistent navigation.')}</p>
                        </div>
                ...
                <section className="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-sm">
                    <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                        ...
                        <Button label={t('Nuevo módulo', 'New module')} icon="pi pi-plus" className="rounded-full" onClick={() => { setEditingModule(emptyModule); setModuleDialog(true); }} />
                    </div>
                    <DataTable value={page.props.modules} dataKey="idModulos" stripedRows className="rounded-2xl border border-slate-200" emptyMessage={t('No hay módulos registrados.', 'No modules registered yet.')}>
                        ...
                        <Column header={t('Acciones', 'Actions')} body={(row: ModuleRow) => <div className="flex gap-2"><Button icon="pi pi-pencil" text onClick={() => { setEditingModule({ ...row, translations: row.translations ?? { es: row.nmodulo, en: '' } }); setModuleDialog(true); }} /><Button icon="pi pi-trash" text severity="danger" onClick={() => router.delete(`/admin/navigation/modules/${row.idModulos}`, { preserveScroll: true, onSuccess: () => showToast('success', t('Módulo eliminado.', 'Module deleted.')) })} /></div>} />
                    </DataTable>
                </section>
                ...
                <section className="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-sm">
                    <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                        <div>
                            ...
                            <p className="text-sm text-slate-600">{t('Cada menú puede pertenecer a un módulo, colgar de un padre y definir etiquetas por idioma.', 'Each menu can belong to a module, attach to a parent and define per-language labels.')}</p>
                        </div>
                        <Button label={t('Nuevo menú', 'New menu')} icon="pi pi-plus" className="rounded-full" onClick={() => { setEditingMenu(emptyMenu); setMenuDialog(true); }} />
                    </div>
                    <DataTable value={page.props.menus} dataKey="id" stripedRows className="rounded-2xl border border-slate-200" emptyMessage={t('No hay menús administrativos registrados.', 'No admin menus registered yet.')}>
                        <Column field="id" header="ID" sortable />
                        <Column header={t('Módulo', 'Module')} body={(row: MenuRow) => moduleMap.get(row.idModulos) ?? row.idModulos} sortable />
                        ...
                        <Column header={t('Padre', 'Parent')} body={(row: MenuRow) => row.id_menu ? page.props.parentMenus.find((menu) => menu.id === row.id_menu)?.translations?.[locale] || page.props.parentMenus.find((menu) => menu.id === row.id_menu)?.translations?.es || page.props.parentMenus.find((menu) => menu.id === row.id_menu)?.nombre || `#${row.id_menu}` : t('Raíz', 'Root')} />
                        ...
                        <Column header={t('Acciones', 'Actions')} body={(row: MenuRow) => <div className="flex gap-2"><Button icon="pi pi-pencil" text onClick={() => { setEditingMenu({ ...row, translations: row.translations ?? { es: row.nombre, en: '' } }); setMenuDialog(true); }} /><Button icon="pi pi-trash" text severity="danger" onClick={() => router.delete(`/admin/navigation/menus/${row.id}`, { preserveScroll: true, onSuccess: () => showToast('success', t('Menú eliminado.', 'Menu deleted.')) })} /></div>} />
                    </DataTable>
                </section>
            </div>

            <Dialog header={editingModule.idModulos ? t('Editar módulo', 'Edit module') : t('Nuevo módulo', 'New module')} visible={moduleDialog} onHide={() => setModuleDialog(false)} className="w-full max-w-2xl">
                <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <label className="space-y-2"><span className="text-sm font-medium text-slate-700">{t('Nombre en español', 'Spanish label')}</span><InputText value={editingModule.translations?.es ?? ''} onChange={(event) => setEditingModule((prev) => ({ ...prev, translations: { ...(prev.translations ?? {}), es: event.target.value }, nmodulo: event.target.value }))} /></label>
                    <label className="space-y-2 md:col-span-2"><span className="text-sm font-medium text-slate-700">{t('Nombre en inglés', 'English label')}</span><InputText value={editingModule.translations?.en ?? ''} onChange={(event) => setEditingModule((prev) => ({ ...prev, translations: { ...(prev.translations ?? {}), en: event.target.value } }))} /></label>
                    ...
                    <div className="col-span-full flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3"><span className="text-sm font-medium text-slate-700">{t('Módulo activo', 'Active module')}</span><InputSwitch checked={!!editingModule.activo} onChange={(event) => setEditingModule((prev) => ({ ...prev, activo: !!event.value }))} /></div>
                </div>
                <div className="mt-5 flex justify-end"><Button label={t('Guardar módulo', 'Save module')} className="rounded-full" onClick={submitModule} /></div>
            </Dialog>

            <Dialog header={editingMenu.id ? t('Editar menú', 'Edit menu') : t('Nuevo menú', 'New menu')} visible={menuDialog} onHide={() => setMenuDialog(false)} className="w-full max-w-3xl">
                <div className="grid gap-4 md:grid-cols-2">
                    <label className="space-y-2"><span className="text-sm font-medium text-slate-700">{t('Módulo', 'Module')}</span><Dropdown value={editingMenu.idModulos} options={moduleOptions} onChange={(event) => setEditingMenu((prev) => ({ ...prev, idModulos: event.value }))} className="w-full" placeholder={t('Selecciona un módulo', 'Select module')} /></label>
                    <label className="space-y-2"><span className="text-sm font-medium text-slate-700">{t('Nombre en español', 'Spanish label')}</span><InputText value={editingMenu.translations?.es ?? ''} onChange={(event) => setEditingMenu((prev) => ({ ...prev, translations: { ...(prev.translations ?? {}), es: event.target.value }, nombre: event.target.value }))} /></label>
                    <label className="space-y-2 md:col-span-2"><span className="text-sm font-medium text-slate-700">{t('Nombre en inglés', 'English label')}</span><InputText value={editingMenu.translations?.en ?? ''} onChange={(event) => setEditingMenu((prev) => ({ ...prev, translations: { ...(prev.translations ?? {}), en: event.target.value } }))} /></label>
                    ...
                    <label className="space-y-2"><span className="text-sm font-medium text-slate-700">{t('Menú padre', 'Parent menu')}</span><Dropdown value={editingMenu.id_menu ?? null} options={parentOptions} onChange={(event) => setEditingMenu((prev) => ({ ...prev, id_menu: event.value }))} className="w-full" /></label>
                ...
                <div className="mt-5 flex justify-end"><Button label={t('Guardar menú', 'Save menu')} className="rounded-full" onClick={submitMenu} /></div>
            </Dialog>
        </>
    );
}
