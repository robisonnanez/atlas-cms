        ...
        <SiteLayout site={site} menu={menu}>
            <section className="mx-auto grid max-w-6xl gap-10 px-6 py-20 lg:grid-cols-[1.1fr_0.9fr]">
                <div>
                    ...
                    <h1 className="mt-4 text-5xl font-semibold tracking-tight text-slate-950">
                        Publica contenido estructurado con la velocidad de un panel moderno.
                    </h1>
                    <p className="mt-6 max-w-2xl text-lg text-slate-600">{hero?.excerpt ?? 'Control editorial para paginas, posts, media, temas y plugins desde una sola plataforma modular.'}</p>
                    <div className="mt-8 flex flex-wrap gap-3">
                        <Link href="/blog" className="rounded-full bg-slate-950 px-5 py-3 font-medium text-white">Leer el blog</Link>
                        <Link href="/login" className="rounded-full border border-slate-300 px-5 py-3 font-medium text-slate-700">Abrir admin</Link>
                    </div>
                </div>
            ...
            <section className="mx-auto max-w-6xl px-6 pb-20">
                <div className="mb-8 flex items-end justify-between">
                    <div>
                        <p className="text-sm uppercase tracking-[0.35em] text-slate-400">Ultimas publicaciones</p>
                        <h2 className="mt-2 text-3xl font-semibold">Novedades recientes de Atlas</h2>
                    </div>
                </div>
                <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                    {posts.map((post) => (
                        <article key={post.id} className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm">
                            ...
                            <Link href={`/blog/${post.slug}`} className="mt-6 inline-flex text-sm font-medium text-blue-600">Leer articulo</Link>
                        </article>
                    ))}
                </div>
            </section>
