...
function PublicMenuNode({ item, depth = 1 }: { item: SiteMenuItem; depth?: number }) {
    ...
    if (!hasChildren) {
        if (!hasLink(item.url)) {
            return <span className="text-slate-500">{item.label}</span>;
        }

        return (
            <Link href={item.url} target={item.target ?? '_self'} className={depth === 1 ? 'text-slate-600 transition hover:text-slate-950' : 'block rounded-xl px-3 py-2 text-sm text-slate-700 transition hover:bg-slate-50 hover:text-slate-950'}>
                {item.label}
            </Link>
        );
    }

    const labelClass = depth === 1
        ? 'text-slate-600 transition hover:text-slate-950'
        : 'block flex-1 rounded-xl px-3 py-2 text-sm text-slate-700 transition hover:bg-slate-50 hover:text-slate-950';

    const toggleClass = depth === 1
        ? 'inline-flex size-7 items-center justify-center rounded-full text-slate-500 transition hover:bg-slate-100 hover:text-slate-950'
        : 'inline-flex size-8 items-center justify-center rounded-xl text-slate-500 transition hover:bg-slate-100 hover:text-slate-950';
                ...
                <div className={`absolute z-40 ${panelClass}`}>
                    <div className="rounded-2xl border border-slate-200 bg-white p-2 shadow-2xl shadow-slate-200/70">
                        <div className="space-y-1">
                            {hasLink(item.url) ? (
                                <Link href={item.url} target={item.target ?? '_self'} className="block rounded-xl px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50 hover:text-slate-950">
                                    Ir a {item.label}
                                </Link>
                            ) : null}
                ...
                <header className="border-b border-slate-200/80 bg-white/80 backdrop-blur">
                    <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
                        ...
                        <nav className="hidden items-center gap-6 text-sm md:flex">
                            {menu.map((item) => (
                                <PublicMenuNode key={item.id} item={item} />
                            ))}
